		...
		if (Helper.isEmpty(name) || Helper.isEmpty(nickname) || Helper.isEmpty(phone) || Helper.isEmpty(fb) || Helper.isEmpty(location)) {
			...
			tabDaiLy.findOne({'nickname':nickname}, function(err, check){
				if (!!check) {
					...
					try {
						tabDaiLy.create({'name':name, 'nickname':nickname, 'phone':phone, 'location':location, 'fb':fb}, function(errC, dataC) {
							if (!!dataC) {
...
function DaiLy(client, data){
	...
	if(isAdmin == false && aclCard.includes('daily') == false){
...
function NhaMang(client, data){
	...
	let aclCard = client.acls.system && Array.isArray(client.acls.system) ?client.acls.system:[];
	if(isAdmin == false && aclCard.includes('nhamang') == false){
		return client.red({notice:{title:'THẤT BẠI', text:'bạn không có quyền'}});
	}
	if (void 0 !== data.add) {
...
function MenhGia(client, data){
	...
	let aclCard = client.acls.system && Array.isArray(client.acls.system) ?client.acls.system:[];
	if(isAdmin == false && aclCard.includes('MenhGia') == false){
		return client.red({notice:{title:'THẤT BẠI', text:'bạn không có quyền'}});
	}
	if (void 0 !== data.add) {
