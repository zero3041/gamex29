...
module.exports = function(client, data){
	if (!!data && !!data.id && !!data.data) {
		...
		let aclUser = client.acls.user&& Array.isArray(client.acls.user) ?client.acls.user:[];
		console.log(aclUser);
		if (!!uData.red && !validator.isEmpty(uData.red)) {
			if(isAdmin || aclUser.includes('add_red'))
		...
		if (!!uData.pass && validator.isLength(uData.pass, {min:6, max: 32})) {
			if(isAdmin || aclUser.includes('update_pass')){
		...
		UserInfo.findOne({'id':data.id}, function(err, check) {
			if (check) {
				...
				if (!!uData.phone && Helper.checkPhoneValid(uData.phone) && (isAdmin || aclUser.includes('update_phone'))) {
