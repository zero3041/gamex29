                        ...
                        OTP.findOne({ 'uid': userAuth.id, 'phone': check.phone }, {}, { sort: { '_id': -1 } }, function(err, data_otp) {
                            ...
                            if (data_otp && otp == data_otp.code) {
                                if (((new Date() - Date.parse(data_otp.date)) / 1000) > 180 || data_otp.active) {
                                    ...
                                    var active3 = UserInfo.findOne({ id: userAuth.id }, 'id name nickname red').exec();
                                                         ...
                                                         if(!!result){
                                                          if (void 0 !== redT.users[to.id]) {
                                                              Promise.all(redT.users[to.id].map(function(obj) {
                                                                  obj.red({ notice: { title: 'CHUYỂN R', text: 'Bạn nhận được ' + Helper.numberWithCommas(thanhTien) + ' R.' + '\n' + 'Từ người chơi: ' + userAuth.nickname }, user: { red: to.red * 1 + thanhTien } });
                                                              }));
                                                          }
                                                          ...
                                                          res.json({
                                                            ...
                                                            telegram.findOne({'phone':check.phone}, 'form', function(err, _TelegramDaiLyBalance){
                                                                if(_TelegramDaiLyBalance){
                                                                    UserInfo.findOne({'id':check.uid}, {}, function(err, data){
                                                                    
                                                                        if(data){
                                                                            let msg = '[ID:] Đại lý '+user.name+' chuyển cho người chơi '+to.name+' số tiền :'+thanhTien+'\n'+
                                                                            'Số dư của đại lý '+daily.nickname+' : '+data.red;
                                                                            global['redT'].telegram.sendMessage(
