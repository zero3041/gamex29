...
module.exports = function(client, data) {
	...
	var file = require('../../../../../config/xocxoc.json');
	var TT   = null;
	if (data == '0') {
		TT = file.bot = false;
	}else if (data == '1') {
	...
	if (TT != null) {	
		fs.writeFile(path.dirname(path.dirname(path.dirname(path.dirname(path.dirname(__dirname))))) + '/config/xocxoc.json', JSON.stringify(file), function(err){
			if (!!err) {
