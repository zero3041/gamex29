		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || client.LongLan.free === 0 && user.red < tongCuoc) {
					client.red({longlan:{status:0, notice:'Bạn không đủ R để quay.!!'}});
				}else{
					...
					HU.findOne({game:'long', type:bet}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											client.redT.sendInHome({pushnohu:{title:'Đêm Thượng Hải', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								isBigWin = true;
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Đêm Thượng Hải', users:client.profile.name, bet:bet_win, status:2}}});
							}
							if (free > 0) {
