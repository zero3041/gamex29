...
function getindex(arrray , name){
	for(let i =0 ; i < arrray.length ; i++){
		console.log(arrray[i].name+" demo "+name); 
					...
					if (!validator.isLength(str, {min:1, max:250})) {
						...
						TXChat.findOne({}, 'uid value', {sort:{'_id':-1}}, function(err, post) {
							if (!post || post.uid != client.UID || (post.uid == client.UID && post.value != str)) {
								...
								let content = {taixiu:{chat:{message:{user:client.profile.name, value:str,top:getindex(client.redT.listTop,client.profile.name)}}}};
						...
						TXCuocOne.findOne({uid:client.UID, phien:phien}, 'bet select', function(isCuocErr, isCuoc) {
							if (!!isCuoc) {
								...
								if (isCuoc.select !== select) {
									...
									tran.create({uid:client.UID,type:"bet",game_session:client.redT.TaiXiu_phien,name:user.name,cash:bet*-1,current_cash:user.red,cnid:"TX",platform:"android",date:new Date()});
									Push.create({
										type:"GameTXBet",
								...
								Push.create({
									type:"GameTXBet",
									data:JSON.stringify({uid:client.UID,money:bet,select:select})
								});
