									...
									try {
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date()}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'Chào mừng bạn đến với X29.CLUB - Cổng game hàng đầu Châu Á, chúc bạn chơi game vui vẻ...', 'time':new Date()});
												GameState(client);
												client.red(data);
											}
										});
