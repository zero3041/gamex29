			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						//if(okHu>45000000)
						io.sendInHome({pushnohu:{title:'Đêm Thượng Hải', name:user.name, bet:okHu}});
						LongLan_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
						nohu = true;
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'Thần Long', users:user.name, bet:bet_win, status:2}}});
				LongLan_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
			}
